...
**`--error-model`** (required) — JSON error model produced by `learn-errors`, or converted from an R DADA2 `learnErrors()` `.rds` with [`learnerrors_to_dada2rs.R`](../scripts/pipeline-helpers.md#learnerrors_to_dada2rsr).
